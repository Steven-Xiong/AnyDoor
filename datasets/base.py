...
class BaseDataset_t2i(Dataset):
    ...
    #这里比较关键
    def process_pairs(self, ref_image, ref_mask, tar_image, tar_mask, layout, max_ratio = 0.8):
        assert mask_score(ref_mask) > 0.90
        assert self.check_mask_area(ref_mask) == True
        assert self.check_mask_area(tar_mask)  == True
        ...
        masked_ref_image = ref_image * ref_mask_3 + np.ones_like(ref_image) * 255 * (1-ref_mask_3) #(428, 640, 3) masked_ref_image.png
        ...
        item = dict(
                # ref = layout.copy(),
                ref=masked_ref_image_aug.copy(),  #（224,224,3）
        ...
        return item
    # 对于lvis 的customization
    def process_pairs_customized(self, ref_image, ref_mask, tar_image, tar_mask, layout, max_ratio = 0.8):
        assert mask_score(ref_mask) > 0.90
        assert self.check_mask_area(ref_mask) == True
        assert self.check_mask_area(tar_mask)  == True
        ...
        # import pdb; pdb.set_trace()
        collage_mask = cropped_target_image.copy() * 0.0   #不用反转了？
        collage_mask[y1:y2,x1:x2,:] = 1.0
        ...
        item = dict(
                # ref=ref_image.copy(),  # masked_ref_image_aug.copy()（224,224,3）
                jpg=cropped_target_image.copy(),  # (512,512,3)
                hint=collage.copy(),  #对应论文中的没HF-Map的collage，在cldm里面加的HF-Map (512, 512, 4). 和mask concat之后的结果
                extra_sizes=np.array([H1, W1, H2, W2]), 
                tar_box_yyxx_crop=np.array(tar_box_yyxx_crop), 
    ...
    def process_pairs_customized_mvimagenet(self, ref_image, ref_mask, tar_image, tar_mask, layout, max_ratio = 0.8):
        assert mask_score(ref_mask) > 0.90
        assert self.check_mask_area(ref_mask) == True
        assert self.check_mask_area(tar_mask)  == True
