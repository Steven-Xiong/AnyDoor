...
def inference_single_image_new(item, guidance_scale = 5.0):
    ...
    cond = {"c_concat": [control], "c_crossattn": [c], "objs" : [c_txt_all]}
    # import pdb; pdb.set_trace()

    un_cond = {"c_concat": None if guess_mode else [control], "c_crossattn": [model.get_unconditional_conditioning(num_samples).to(device)],"objs": [c_txt_all]}
    shape = (4, H // 8, W // 8)
