...
def fire_clip_before_after(loader, folder):
    ...
    for batch in tqdm(loader):
        ...
        inputs = processor(text=batch['txt'],  return_tensors="pt", padding=True) #为什么得到的shape不一样？
        inputs['input_ids'] = inputs['input_ids'].cuda()
        inputs['pixel_values'] = batch['ref'].permute(0,3,1,2).cuda() # we use our own preprocessing without center_crop 
        ...
        for idx, text_before, text_after, image_before, image_after  in zip(batch["anno_id"], text_before_features, text_after_features, image_before_features, image_after_features):
            # import pdb; pdb.set_trace()
            idx = idx.replace('.jpg','')
            save_name = os.path.join(folder, 'text_features_before', str(idx)) 
            os.makedirs( save_name,  exist_ok=True   )
            torch.save(text_before.clone().cpu(), save_name+'.npz')

            save_name = os.path.join(folder, 'text_features_after', str(idx) )
            os.makedirs( save_name,  exist_ok=True   )
            torch.save(text_after.clone().cpu(), save_name+'.npz')

            save_name = os.path.join(folder, 'image_features_before', str(idx) )
            os.makedirs( save_name,  exist_ok=True   )
            torch.save(image_before.clone().cpu(), save_name+'.npz')

            save_name = os.path.join(folder, 'image_features_after', str(idx) )
            os.makedirs( save_name,  exist_ok=True   )
            torch.save(image_after.clone().cpu(), save_name+'.npz')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json_path", type=str,  default="/project/osprey/scratch/x.zhexiao/edit/LayoutBooth/AnyDoor/data/coco/annotations/captions_train2017.json", help="")
    parser.add_argument("--image_root", type=str,  default="/project/osprey/scratch/x.zhexiao/edit/LayoutBooth/AnyDoor/data/coco/images/train2017/", help="")
    parser.add_argument("--folder", type=str,  default="/project/osprey/scratch/x.zhexiao/edit/LayoutBooth/AnyDoor/DATA/mvimgnet_tmp", help="")
    ...
    # GroundedTextImageDataset_Detection(args.json_path, args.image_root, args.chunk_idx, args.total_chunk).getitem(1)
    # dataset = GroundedTextImageDataset_Detection(args.json_path, args.image_root, args.chunk_idx, args.total_chunk)
    # if args.is_o365:
    #     dataset = GroundedTextImageDataset_Detection(args.json_path, args.image_root, args.chunk_idx, args.total_chunk)
    # else:
    #     GroundedTextImageDataset_Grounding(args.json_path, args.image_root, args.chunk_idx, args.total_chunk).getitem(1)
